...
export class SpecialsDetector {
  ...
  public static analyzeSpecials(
    ...
    const targetSeasons = this.determineTargetSeasons(specialEpisodes, regularSeasons);
    const chronologicalPosition = this.determineChronologicalPosition(specialEpisodes, regularSeasons);
    ...
    const episodeMappings = 
      integrationStrategy === 'distributed' 
   ...
   */
  private static hasSpreadOutAirDates(specialEpisodes: SkyhookEpisode[]): boolean {
  ...
  private static determineChronologicalPosition(
    ...
    const hasBeforeSeason = specialEpisodes.some(ep => ep.airedBeforeSeasonNumber !== undefined);
    const hasAfterSeason = specialEpisodes.some(ep => ep.airedAfterSeasonNumber !== undefined);
    ...
    const minSpecialDate = new Date(Math.min(...specialDates.map(d => d.getTime())));
    const maxSpecialDate = new Date(Math.max(...specialDates.map(d => d.getTime())));
    ...
    const minSeasonDate = new Date(Math.min(...seasonDates.map(d => d.getTime())));
    const maxSeasonDate = new Date(Math.max(...seasonDates.map(d => d.getTime())));
  ...
  private static createEpisodeMappings(
    ...
    specialEpisodes.forEach((episode, index) => {
      if (episode.airedBeforeSeasonNumber !== undefined && episode.airedBeforeEpisodeNumber !== undefined) {
      ...
      if (episode.airedAfterSeasonNumber !== undefined && episode.airedAfterEpisodeNumber !== undefined) {
        mappings[episode.episodeNumber] = {
      ...
      if (episode.airDate) {
        ...
        const closestSeason = this.findClosestSeason(specialDate, regularSeasons);
        if (closestSeason) {
          mappings[episode.episodeNumber] = {
      ...
      if (!mappings[episode.episodeNumber]) {
        ...
        const targetSeasonNum = regularSeasons[index % regularSeasons.length]?.season_number || 1;
        mappings[episode.episodeNumber] = {
