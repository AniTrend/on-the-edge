...
Scope and composition of instructions

- This file contains cross-cutting standards that apply repo-wide. Module-specific rules live in sibling files under `.github/instructions/*` with folder-scoped `applyTo` globs (e.g., `src/service/**`, `src/database/**`).
- The instruction system composes all files whose `applyTo` match the current task. This keeps context lean: you’ll only get module-specific guidance when working in those areas.
...
- `src/setup.ts`, `src/app.module.ts`
  - Entry points for server bootstrap and setup definitions.
- `src/common/`
  - Shared core: environment loading, logging, OTEL tracing, middleware, MongoDB factory.
- `src/secret/`
  - Environment variable sourcing wrapped and isolated for ease of testing.
- `src/package/*`
...
6. Follow error-handling and logging patterns:
   - Use `logger.service.ts` -> `LoggerService` module.
7. Handle configuration securely:
   - Load via `secret.service.ts`, use factories, never hard-code secrets.
...
- Model runtime validation with `zod` schemas that mirror our DTO types; keep parsing close to the boundary that ingests external input.
- Prefer `schema.nullish()` over `schema.optional()` so the validator always emits keys; reserve `.optional()` for fields that truly may be omitted.
- Use `.default()` with `.nullish()` when a fallback value makes downstream logic simpler; document defaults inline via code comments if they differ from backend contracts.
- Prefer schema-driven `.transform()` steps for simple shape adjustments instead of ad-hoc functional transformations inside callers.
- Compose reusable fragments via `.extend()` or helpers stored alongside the owning module to avoid duplicated rules.
...
2. Apply one logical change per patch; group multi-file updates in a single with summary.
3. Validate edits with `deno fmt`, `deno lint`, `deno task test --filter` and report outcomes.
4. Include unit and integration tests (happy path + edge case) for new behaviors.

## 5. Quality Gates

- **Build**: run `deno task cache` after adding dependencies.
...
- Default to Deno CLI (`deno task dev`) if runtime unspecified.
...
- Never read or write into `.env` instead use `.env.example`
- Do not commit secrets; use `secret.service.ts` for secret management and document required variables.
- Avoid outbound network calls in tests; `@c4spar/mock-fetch` along with the `@scope/client` package can be used to mock fetch requests.
...
- **Add a new domain module** (typically controllers for the API surface):
  1.  Create `src/package/<domain>/{<name>.service.ts,<name>.controller.ts,<name>.module.ts}` e.t.c along with any integration test in `./sepc`.
  2.  Update `package.module.ts` with the relevant imports and exports for dependency injection.
  3.  Implement repositories/transformer and other sub-modules as needed follow the repository patterns.
...
- OTEL & Tracing: core configuration and setup `src/telemetry/telemetry.service.ts`, `src/telemetry/telemetry.factory.ts` and `src/middleware/tracing.middleware.ts` for distributed tracing.
- Logging: `src/logger/logger.service.ts`.
- Domain examples: `src/packages/config`, `src/packages/*`.
...
### API & pagination contracts (packages/services)
...
### Experiments & feature flags (experiment)

- Represent features via a `Features` interface (GrowthBook-style). Inject it into modules that need gated behavior. Default OFF in tests and local dev; explicitly enable within tests to validate toggled behavior. Never gate behavior directly on raw environment variables inside business logic—use secert sources and the features provider.

### Dependency injection & persistence (database, package)

- Constructors/functions accept explicit dependencies (databases, services, repositories, e.t.c). Avoid reading globals or singletons directly in business logic. Use interfaces and adapters for persistence (e.g., `Collection<T>` interface with Mongo and in-memory implementations).

### Observability (logger, telemetry, middleware)
